import React, { SVGAttributes } from 'react';

export const TrashIcon = (props: SVGAttributes<SVGSVGElement>) => (
  <svg
    width={24}
    height={24}
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    {...props}
  >
    <path d="M0 0h24v24H0V0Z" fill="url(#trash_svg__a)" />
    <defs>
      <pattern id="trash_svg__a" patternContentUnits="objectBoundingBox" width={1} height={1}>
        <use xlinkHref="#trash_svg__b" transform="scale(.025)" />
      </pattern>
      <image
        id="trash_svg__b"
        width={40}
        height={40}
        xlinkHref="data:image/png;base64,"
      />
    </defs>
  </svg>
);
